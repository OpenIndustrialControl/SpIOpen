...
LifecycleState FrameMessageAllocator::GetState() const {
    const auto& children = GetChildComponents();
    const LifecycleState cc_state = etl::get<kCcPoolIndex>(children).GetState();
    const LifecycleState fd_state = etl::get<kFdPoolIndex>(children).GetState();
    const LifecycleState xl_state = etl::get<kXlPoolIndex>(children).GetState();
    if ((fd_state != cc_state) || (xl_state != cc_state)) {
...
FramePool* FrameMessageAllocator::SelectPoolForCanMessageType(format::CanMessageType can_message_type) {
    ...
    switch (can_message_type) {
        ...
        case format::CanMessageType::CanFd:
            return BROKER_CAN_FD_ENABLED ? &etl::get<kFdPoolIndex>(children) : nullptr;
        case format::CanMessageType::CanXl:
            return BROKER_CAN_XL_ENABLED ? &etl::get<kXlPoolIndex>(children) : nullptr;
        default:
            return nullptr;
    }
}

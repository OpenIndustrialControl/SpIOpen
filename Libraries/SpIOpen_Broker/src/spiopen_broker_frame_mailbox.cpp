/*
SpIOpen Broker Frame Mailbox : Implementation.
...
bool IsInitializedMailboxState(LifecycleState state) {
    return (state == LifecycleState::Inactive) || (state == LifecycleState::Starting) ||
           (state == LifecycleState::Active) || (state == LifecycleState::Stopping) ||
...
etl::expected<size_t, FrameMailboxError> FrameMailbox::DrainAndReleaseAll() {
    const LifecycleState current_state = state_.load(std::memory_order_relaxed);
    if (!IsInitializedMailboxState(current_state)) {
        return etl::unexpected(FrameMailboxError::NotInitialized);
    }
...
etl::expected<void, FrameMailboxError> FrameMailbox::Enqueue(FrameMessage* message, uint32_t timeout_ticks) {
    ...
    if (current_state != LifecycleState::Active) {
        return etl::unexpected(FrameMailboxError::NotActive);
    }

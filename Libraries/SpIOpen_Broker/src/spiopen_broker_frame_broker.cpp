/*
SpIOpen Broker Frame Broker : Implementation.
...
etl::expected<FrameBroker::ConfigType, FrameBroker::ErrorType> FrameBroker::ValidateAndNormalizeConfiguration(
    ...
    if ((config.thread_stack_size_bytes == 0U) || (config.thread_stack_size_bytes > BROKER_THREAD_MAX_STACK_SIZE)) {
        return etl::unexpected(LifecycleError(LifecycleErrorType::InvalidConfiguration));
    }
    if (!BROKER_THREAD_STACK_MEMORY_CONFIGURABLE && (config.thread_stack_size_bytes != BROKER_THREAD_MAX_STACK_SIZE)) {
        return etl::unexpected(LifecycleError(LifecycleErrorType::InvalidConfiguration));
    }
    if (!config.thread_stack_storage.empty() && (config.thread_stack_storage.size() < config.thread_stack_size_bytes)) {
        return etl::unexpected(LifecycleError(LifecycleErrorType::InvalidConfiguration));
    }
    if (!BROKER_THREAD_STACK_MEMORY_CONFIGURABLE && config.thread_stack_storage.empty()) {
        return etl::unexpected(LifecycleError(LifecycleErrorType::InvalidConfiguration));
    }
    ...
    if (!BROKER_THREAD_STACK_MEMORY_CONFIGURABLE) {
        normalized_config.thread_stack_size_bytes = BROKER_THREAD_MAX_STACK_SIZE;
    }
...
etl::expected<void, LifecycleError> FrameBroker::Reset() {
    ...
    state_.store(LifecycleState::Resetting, std::memory_order_release);

    auto mailbox_reset_ret = inbox_mailbox_.Reset();
    if (!mailbox_reset_ret) {
        state_.store(LifecycleState::Configured, std::memory_order_release);
        return etl::unexpected(LifecycleError(mailbox_reset_ret.error().error));
    }
    subscribers_.fill(nullptr);

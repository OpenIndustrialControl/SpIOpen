...
TEST(SpIOpen_Broker_FrameMessage, Constructor) {
    FakeFramePool pool;
...

    {
        ...
        EXPECT_EQ(message.GetMessageType(), message::MessageType::None);
...
TEST(SpIOpen_Broker_FrameMessage, AllocateToPublisher) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
    ...
    {
        auto ret = message.AllocateToPublisher(message::MessageType::MasterToSlave, &publisher, 1U);
        ...
        EXPECT_EQ(message.GetMessageType(), message::MessageType::MasterToSlave);
        EXPECT_EQ(message.GetPublisherHandle(), &publisher);
    }

    {
        auto ret = message.AllocateToPublisher(message::MessageType::SlaveToMaster, nullptr, 1U);
...

    {
        ...
        auto ret = fresh_message.AllocateToPublisher(message::MessageType::MasterToSlave, &publisher, 0U);
...

    {
        ...
        auto ret = message2.AllocateToPublisher(message::MessageType::MasterToSlave, nullptr, 1U);
...
TEST(SpIOpen_Broker_FrameMessage, LockForPublishing) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, AcquireReference) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...

    {
        ...
        ASSERT_TRUE(overflow_message.AllocateToPublisher(message::MessageType::MasterToSlave, nullptr, UINT8_MAX));
...
TEST(SpIOpen_Broker_FrameMessage, GetMutableFrame) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, GetMutableFrameBuffer) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, GetFrame) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, GetFrameBuffer) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, GetMessageType) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
    ...
    {
        ASSERT_TRUE(message.AllocateToPublisher(message::MessageType::SlaveToMaster, nullptr, 1U));
        EXPECT_EQ(message.GetMessageType(), message::MessageType::SlaveToMaster);
    }
}

TEST(SpIOpen_Broker_FrameMessage, GetPublisherHandle) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
    ...
    {
        ASSERT_TRUE(message.AllocateToPublisher(message::MessageType::MasterToSlave, &publisher, 1U));
        EXPECT_EQ(message.GetPublisherHandle(), &publisher);
    }
}

TEST(SpIOpen_Broker_FrameMessage, GetState) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, GetReferenceCount) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
...
TEST(SpIOpen_Broker_FrameMessage, Release) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));
    ...
    {
        ASSERT_TRUE(message.AllocateToPublisher(message::MessageType::MasterToSlave, nullptr, 2U));
        ASSERT_TRUE(message.LockForPublishing());
        EXPECT_EQ(message.GetReferenceCount(), 2U);
        EXPECT_EQ(message.GetState(), FrameMessageState::Published);
    }
...
TEST(SpIOpen_Broker_FrameMessage, GetOwningPool) {
    FakeFramePool pool;
    std::array<uint8_t, 64U> storage = {};
    FrameMessage message(pool, etl::span<uint8_t>(storage.data(), storage.size()));

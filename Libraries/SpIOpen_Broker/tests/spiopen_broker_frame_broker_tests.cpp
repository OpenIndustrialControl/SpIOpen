...
constexpr size_t kThreadStackBytes = BROKER_THREAD_MAX_STACK_SIZE;
...
TEST(SpIOpen_Broker_FrameBroker, Constructor) {
    FrameBroker broker;
    EXPECT_EQ(broker.GetState(), LifecycleState::Unconfigured);
    EXPECT_EQ(broker.GetEnqueueErrorCount(), 0U);
}

TEST(SpIOpen_Broker_FrameBroker, ConfigureAcceptsValidConfig) {
    BrokerHarness h;
...
TEST(SpIOpen_Broker_FrameBroker, ConfigureRejectsInvalidArgs) {
    {
        BrokerHarness h;
    ...
    {
        BrokerHarness h;
        h.config.thread_stack_size_bytes = static_cast<uint32_t>(BROKER_THREAD_MAX_STACK_SIZE + 1U);
        auto ret = h.broker.Configure(h.config);
...
TEST(SpIOpen_Broker_FrameBroker, ConfigureRequiresExternalStackWhenNotConfigurable) {
    if constexpr (!BROKER_THREAD_STACK_MEMORY_CONFIGURABLE) {
        BrokerHarness h;
...
TEST(SpIOpen_Broker_FrameBroker, InitializeAllocatesStackWhenConfigurableAndMissingBuffer) {
    if constexpr (BROKER_THREAD_STACK_MEMORY_CONFIGURABLE) {
        BrokerHarness h;
        h.config.thread_stack_storage = etl::span<uint8_t>();
...
TEST(SpIOpen_Broker_FrameBroker, ConfigureRejectsWrongState) {
    BrokerHarness h;
...
TEST(SpIOpen_Broker_FrameBroker, FullLifecycle) {
    BrokerHarness h;
    ...
    ASSERT_TRUE(h.broker.Reset());
    EXPECT_EQ(h.broker.GetState(), LifecycleState::Unconfigured);
}

TEST(SpIOpen_Broker_FrameBroker, SubscribeAndUnsubscribe) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());

    SubscriberHarness sub("sub1", static_cast<uint16_t>(message::MessageType::MasterToSlave));
...
TEST(SpIOpen_Broker_FrameBroker, SubscribeRejectsWhenActive) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
    ASSERT_TRUE(h.broker.Start());
...
TEST(SpIOpen_Broker_FrameBroker, SubscribeRejectsNullHandle) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
...
TEST(SpIOpen_Broker_FrameBroker, SubscribeRejectsNullMailbox) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
...
TEST(SpIOpen_Broker_FrameBroker, SubscribeTableFull) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
...
TEST(SpIOpen_Broker_FrameBroker, UnsubscribeNotFound) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
...
TEST(SpIOpen_Broker_FrameBroker, PublishRequiresActiveState) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
    EXPECT_EQ(h.broker.GetState(), LifecycleState::Inactive);
    ...
    ASSERT_TRUE(message.AllocateToPublisher(message::MessageType::MasterToSlave, nullptr, 1U));
...
TEST(SpIOpen_Broker_FrameBroker, PublishRejectsNullMessage) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
    ASSERT_TRUE(h.broker.Start());
...
TEST(SpIOpen_Broker_FrameBroker, PublishEnqueuesToInbox) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
    ASSERT_TRUE(h.broker.Start());
...
TEST(SpIOpen_Broker_FrameBroker, ResetClearsSubscribers) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
    ...
    auto ret = h.broker.Unsubscribe(&sub.handle);
    EXPECT_FALSE(ret) << "After Reset, old subscriber should not be found";
    if (!ret) {
        EXPECT_EQ(ret.error(), FrameBrokerError::SubscriptionNotFound);
    }
}

TEST(SpIOpen_Broker_FrameBroker, StopDrainsInbox) {
    BrokerHarness h;
    ASSERT_TRUE(h.ConfigureAll());
    ASSERT_TRUE(h.broker.Initialize());
    ASSERT_TRUE(h.broker.Start());
    ...
    ASSERT_TRUE(msg1.AllocateToPublisher(message::MessageType::MasterToSlave, nullptr, 1U));
    ASSERT_TRUE(msg2.AllocateToPublisher(message::MessageType::MasterToSlave, nullptr, 1U));

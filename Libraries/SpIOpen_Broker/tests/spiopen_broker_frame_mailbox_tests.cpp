...
TEST(SpIOpen_Broker_FrameMailbox, Constructor) {
    FrameMailbox mailbox;

    EXPECT_EQ(mailbox.GetState(), LifecycleState::Unconfigured);
}

TEST(SpIOpen_Broker_FrameMailbox, Configure) {
    FrameMailbox mailbox;
...
TEST(SpIOpen_Broker_FrameMailbox, ConfigureRejectsInvalidArguments) {
    FrameMailbox mailbox;
...
TEST(SpIOpen_Broker_FrameMailbox, Lifecycle) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
...
TEST(SpIOpen_Broker_FrameMailbox, ResetFromConfigured) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size()), "mailbox-reset"};

    ASSERT_TRUE(mailbox.Configure(config));
    auto ret = mailbox.Reset();
    ASSERT_TRUE(ret) << "Reset should succeed from Configured state";
    EXPECT_EQ(mailbox.GetState(), LifecycleState::Unconfigured);
}

TEST(SpIOpen_Broker_FrameMailbox, ResetInvalidState) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ...
    {
        auto ret = mailbox.Reset();
        EXPECT_FALSE(ret) << "Reset should fail when already Unconfigured";
        if (!ret) {
            EXPECT_EQ(ret.error().error, LifecycleErrorType::NotConfigured);
        }
    }
    ...
    {
        auto ret = mailbox.Reset();
        EXPECT_FALSE(ret) << "Reset should fail while initialized";
        if (!ret) {
...
TEST(SpIOpen_Broker_FrameMailbox, EnqueueRequiresActiveState) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ASSERT_TRUE(mailbox.Configure(config));
    ...
    if (!ret) {
        EXPECT_EQ(ret.error(), FrameMailboxError::NotActive);
    }
}

TEST(SpIOpen_Broker_FrameMailbox, EnqueueRejectsNullMessage) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ASSERT_TRUE(mailbox.Configure(config));
...
TEST(SpIOpen_Broker_FrameMailbox, DequeueAllowedWhileInactive) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ASSERT_TRUE(mailbox.Configure(config));
...
TEST(SpIOpen_Broker_FrameMailbox, DrainAndReleaseAllAllowedWhileInactive) {
    FrameMailbox mailbox;
    std::array<uint8_t, 8U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{8U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ASSERT_TRUE(mailbox.Configure(config));
...
TEST(SpIOpen_Broker_FrameMailbox, EnqueueAcquireAndRollbackOnFailure) {
    FrameMailbox mailbox;
...
TEST(SpIOpen_Broker_FrameMailbox, DequeueIsMoveAndDoesNotReleaseReference) {
    FrameMailbox mailbox;
    std::array<uint8_t, 1U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{1U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ASSERT_TRUE(mailbox.Configure(config));
...
TEST(SpIOpen_Broker_FrameMailbox, DeinitializeFailsWhenMailboxIsNotEmpty) {
    FrameMailbox mailbox;
    std::array<uint8_t, 1U * sizeof(FrameMessage*)> queue_storage = {};
    FrameMailboxConfig config{1U, etl::span<uint8_t>(queue_storage.data(), queue_storage.size())};
    ASSERT_TRUE(mailbox.Configure(config));

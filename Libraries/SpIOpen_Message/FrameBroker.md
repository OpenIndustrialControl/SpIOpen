...
- A broker inbox mailbox depth of zero is valid and intentionally enables direct-publish mode for testing. In this mode, no broker inbox queue is instantiated and `Publish()` directly executes subscriber fan-out on the caller thread.

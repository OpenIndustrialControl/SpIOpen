/*
SpIOpen Message Frame Message : Used to pass frames between publishers and subscribers within the broker.
...
#if (CONFIG_SPIOPEN_MESSAGE_MAX_SUBSCRIBER_COUNT > 255)
#error \
    "CONFIG_SPIOPEN_MESSAGE_MAX_SUBSCRIBER_COUNT must fit in uint8_t (<= 255). Adjust Kconfig range or refcount type."
#endif
static constexpr size_t MESSAGE_MAX_SUBSCRIBER_COUNT = CONFIG_SPIOPEN_MESSAGE_MAX_SUBSCRIBER_COUNT;
#else
static constexpr size_t MESSAGE_MAX_SUBSCRIBER_COUNT = 8U;
#endif
...
class FrameMessage final {
   ...
   private:
    bool IsState(FrameMessageState expected, std::memory_order order = std::memory_order_relaxed) const {

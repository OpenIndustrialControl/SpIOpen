...
TEST(SpIOpen_Message_FramePool, Constructor) {
    FramePool pool;
...
TEST(SpIOpen_Message_FramePool, BytesToAllocateForFramePool) {
...

    {
        const size_t expected =
            messages_offset + messages_storage_size +
            (message_count *
             format::MAX_CAN_MESSAGE_FRAME_SIZE_BY_TYPE[static_cast<size_t>(format::CanMessageType::CanCc)]);
...

    {
        const size_t expected =
            messages_offset + messages_storage_size +
            (message_count *
             format::MAX_CAN_MESSAGE_FRAME_SIZE_BY_TYPE[static_cast<size_t>(format::CanMessageType::CanFd)]);
...

    {
        const size_t expected =
            messages_offset + messages_storage_size +
            (message_count *
             format::MAX_CAN_MESSAGE_FRAME_SIZE_BY_TYPE[static_cast<size_t>(format::CanMessageType::CanXl)]);
...
TEST(SpIOpen_Message_FramePool, ConfigureAndLifecycle) {
    FramePool pool;
    std::array<uint8_t, kCcPoolBytes> pool_storage{};
    FramePoolConfig config{kCcMessageCount, format::MAX_CAN_CC_FRAME_SIZE,
                           etl::span<uint8_t>(pool_storage.data(), pool_storage.size())};
    ...
    auto unconfigure_ret = pool.Unconfigure();
    ASSERT_TRUE(unconfigure_ret);
    EXPECT_EQ(pool.GetState(), LifecycleState::Unconfigured);
}

TEST(SpIOpen_Message_FramePool, ConfigureRejectsInsufficientStorage) {
    FramePool pool;
...
TEST(SpIOpen_Message_FramePool, AllocateRequiresActiveState) {
    FramePool pool;
    std::array<uint8_t, kCcPoolBytes> pool_storage{};
    FramePoolConfig config{kCcMessageCount, format::MAX_CAN_CC_FRAME_SIZE,
                           etl::span<uint8_t>(pool_storage.data(), pool_storage.size())};
    ...
    {
        auto ret = pool.AllocateFrameMessage(MessageType::MasterToSlave, nullptr, 0U);
        EXPECT_FALSE(ret);
        if (!ret) {
            EXPECT_EQ(ret.error(), FramePoolError::InvalidState);
        }
    }
...
TEST(SpIOpen_Message_FramePool, AllocateReturnsMetadataAndPoolExhaustedThenRecycle) {
    constexpr size_t kOneMessageCount = 1U;
    ...
    auto first = pool.AllocateFrameMessage(MessageType::MasterToSlave, &publisher, 0U);
    ASSERT_TRUE(first);
    ASSERT_NE(*first, nullptr);
    EXPECT_EQ((*first)->GetOwningPool().GetState(), pool.GetState());
    EXPECT_EQ((*first)->GetMessageType(), MessageType::MasterToSlave);
    ...
    auto second = pool.AllocateFrameMessage(MessageType::MasterToSlave, &publisher, 0U);
    EXPECT_FALSE(second);
    if (!second) {
    ...
    auto third = pool.AllocateFrameMessage(MessageType::SlaveToMaster, nullptr, 0U);
    ASSERT_TRUE(third);
    EXPECT_EQ(*third, *first) << "Single-message pool should recycle the same message object";
    EXPECT_EQ((*third)->GetMessageType(), MessageType::SlaveToMaster);

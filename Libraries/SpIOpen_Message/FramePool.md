...
- spiopen_message_frame_pool.h / .cpp contains the class
...
`FramePool` implements `ILifecycleComponent<FramePoolConfig, LifecycleError>` from `SpIOpen_Lifecycle`.

- `Configure()` validates and normalizes memory sizing and queue configuration.
- `Initialize()` binds/allocates backing storage and constructs queue/message regions.
- `Start()` transitions to active allocation state.
- `Stop()` transitions out of active allocation state.
- `Deinitialize()` validates teardown conditions and releases runtime resources.
- `Reset()` clears configured state after deinitialization.

`FrameMessageAllocator` uses `IAggregateLifecycleComponent<NoLocalConfig, FramePool, ...>` to fan out lifecycle transitions across child pools and report aggregate child errors when one or more pools fail.
...
- In `spiopen_message_frame_pool.h`: `BytesToAllocateForFramePool(message_count, can_message_type)`

...
static constexpr int FRAME_WRITE_ERROR_INVALID_BUFFER_POINTER = -4;   // The buffer pointer is invalid or NULL
static constexpr int FRAME_WRITE_ERROR_INVALID_FRAME_POINTER = -5;    // The frame pointer is invalid or NULL
static constexpr int FRAME_WRITE_ERROR_CANFD_NOT_SUPPORTED = -6;      // CAN-FD not supported by the frame pool
static constexpr int FRAME_WRITE_ERROR_CANXL_NOT_SUPPORTED = -7;      // CAN-XL not supported by the frame pool
...
struct FrameWriteResult {
    ...
    size_t payload_padding_added;  // Total length of the payload padding added to the frame in bytes to meet the Data
                                   // Length Code (DLC) requirements
    size_t frame_padding_added;    // Total length of the frame padding added to the frame in bytes to meet the Word

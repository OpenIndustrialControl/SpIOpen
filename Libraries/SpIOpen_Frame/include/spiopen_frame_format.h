...
namespace spiopen::format {
    ...
    static constexpr size_t XL_CONTROL_SIZE = 8;                     // Size of the XL control field in bytes (Only used if XLF flag is set)
    static constexpr size_t CAN_IDENTIFIER_SIZE = 2;                 // Size of the basic CAN identifier in bytes
    static constexpr size_t CAN_IDENTIFIER_EXTENSION_SIZE = 2;       // Size of additional extended identifier in bytes (Only used if IDE flag is set)
    static constexpr size_t TIME_TO_LIVE_SIZE = 1;                   // Size of the Time to Live counter in bytes (Only used if TTL flag is set)
    ...
    static constexpr size_t MAX_PADDING_SIZE = 1;                    // Maximum size of the padding byte in bytes (Only used if WA flag is set)
    ...
    static constexpr uint8_t HEADER_WA_MASK = 0x01;            // Mask for the word alignment field in the high byte of the format header
    ...
    static constexpr size_t MAX_CAN_CC_HEADER_SIZE = (PREAMBLE_SIZE + FORMAT_HEADER_SIZE + CAN_IDENTIFIER_SIZE + CAN_IDENTIFIER_EXTENSION_SIZE + TIME_TO_LIVE_SIZE)
    static constexpr size_t MAX_CAN_FD_HEADER_SIZE = (PREAMBLE_SIZE + FORMAT_HEADER_SIZE + CAN_IDENTIFIER_SIZE + CAN_IDENTIFIER_EXTENSION_SIZE + TIME_TO_LIVE_SIZE)
    static constexpr size_t MAX_CAN_XL_HEADER_SIZE = (PREAMBLE_SIZE + FORMAT_HEADER_SIZE + CAN_IDENTIFIER_SIZE + CAN_IDENTIFIER_EXTENSION_SIZE + TIME_TO_LIVE_SIZE + XL_CONTROL_SIZE)

    static constexpr size_t MAX_CAN_CC_FRAME_SIZE = (MAX_CAN_CC_HEADER_SIZE + MAX_CC_PAYLOAD_SIZE + SHORT_CRC_SIZE + MAX_PADDING_SIZE)
    static constexpr size_t MAX_CAN_FD_FRAME_SIZE = (MAX_CAN_FD_HEADER_SIZE + MAX_FD_PAYLOAD_SIZE + LONG_CRC_SIZE + MAX_PADDING_SIZE)
    static constexpr size_t MAX_CAN_XL_FRAME_SIZE = (MAX_CAN_XL_HEADER_SIZE + MAX_XL_PAYLOAD_SIZE + LONG_CRC_SIZE + MAX_PADDING_SIZE)
}
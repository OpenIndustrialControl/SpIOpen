/**
 * CANopenNode driver over SpIOpen transport (RP2040 slave).
 * Implements CO_driver.h: frames are sent via chainbus_tx; received frames
    ...
    uint16_t rxSize, CO_CANtx_t txArray[], uint16_t txSize, uint16_t CANbitRate)
{
    ...
    CANmodule->useCANrxFilters = false; /* software match in inject_rx */
...
CO_ReturnError_t CO_CANsend(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer)
{
    ...
    if (tx_buf != NULL) {
        ...
        if (total != 0) {
            if (send_to_chainbus_tx(tx_buf, (uint8_t)total, 0) == pdTRUE) {
                buffer->bufferFull = false;
...
/* Message used when injecting; CO_CANrxMsg_read* access this. */
...
int spiopen_can_driver_inject_rx(uint8_t *buf, size_t len)
{
    ...
    /* Release SpIOpen buffer now; callbacks only see &s_rx_msg, not buf. */
    frame_pool_put(buf);
